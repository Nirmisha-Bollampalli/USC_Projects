...
$sql = mysql_query("(SELECT c.ProductID,c.ProductName,c.ProductImage,c.ProductPrice,s.Discount FROM  cakes c INNER JOIN special_sales s on s.ProductID = c.ProductID and s.EndDate >= '$currentdate' where c.ProductID = '$ID') UNION (SELECT c.ProductID,c.ProductName,c.ProductImage,c.ProductPrice,s.Discount FROM  cakes c INNER JOIN special_sales s on s.ProductID = c.ProductID and s.EndDate >= '$currentdate' where c.ProductID <> '$ID') "); 
...
$sql2 = mysql_query("(SELECT c.ProductID,c.ProductName,c.ProductImage,c.ProductPrice,s.Discount FROM  cakes c INNER JOIN special_sales s on s.ProductID = c.ProductID and s.EndDate >= '$currentdate' where c.ProductID = '$ID') UNION (SELECT c.ProductID,c.ProductName,c.ProductImage,c.ProductPrice,s.Discount FROM  cakes c INNER JOIN special_sales s on s.ProductID = c.ProductID and s.EndDate >= '$currentdate' where c.ProductID <> '$ID') $limit"); 
               ...
               <div>	   
				...
				<div id="categorydiv"  >
						<Table class="first" valign="left" Border="0" Id="product_categories">
									...
									?>
												<TR>
												<td> <a href="sale_category_display.php?ID=<?php echo $f4; ?>"><input type="button" style="width:130px;height:30px;color:blue"  value="<?php echo $f3; ?>" /></a></td>
												</TR>
									<?php
