...
public class MySQLDBClient extends DB implements MySQLDBClientConstants {
	...
	public int acceptFriend(int inviterID, int inviteeID) {
		...
		if(inviterID < 0 || inviteeID < 0)
			return -1;
		String query;
		query = "INSERT INTO friendshipconfirm values(?,?)";
		try {
		...
		String query1= "DELETE FROM friendshippending WHERE inviterid=? and inviteeid= ?";
		try {
			if((preparedStatement = newCachedStatements.get(REJREQ_STMT)) == null)
				preparedStatement = createAndCacheStatement(REJREQ_STMT, query1);
			preparedStatement.setInt(1, inviterID);
			preparedStatement.setInt(2, inviteeID);
			preparedStatement.executeUpdate();
		}catch(SQLException sx){
	...
	public int insertEntity(String entitySet, String entityPK, HashMap<String, ByteIterator> values, boolean insertImage) {
		//System.out.println("I'm in Insert Entity");
		if (entitySet == null) {
			return -1;
		}
		if (entityPK == null) {
	...
	public int viewProfile(int requesterID, int profileOwnerID,
		...
		try {
			//friend count
			query = "SELECT count(*) FROM  friendshipconfirm WHERE (inviterID = ? OR inviteeID = ?)";
			//preparedStatement = conn.prepareStatement(query);
		...
		if(requesterID == profileOwnerID){
			query = "SELECT count(*) FROM  friendshippending WHERE inviteeID = ? ";
			try {
				//preparedStatement = conn.prepareStatement(query);
	...
	public int listFriends(int requesterID, int profileOwnerID,
		...
		try {
			if(insertImage && FSimagePath.equals("")){
				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM users, friendshipconfirm WHERE ((inviterid=? and userid=inviteeid) or (inviteeid=? and userid=inviterid))";
				...
				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM users, friendshipconfirm WHERE ((inviterid=? and userid=inviteeid) or (inviteeid=? and userid=inviterid))";
	...
	public int viewFriendReq(int profileOwnerID,
		...
		try {
			if(insertImage && FSimagePath.equals("")){
				query = "SELECT userid, inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel,tpic FROM users, friendshippending WHERE inviteeid=? and inviterid = userid";
				...
				query = "SELECT userid,inviterid, inviteeid, username, fname, lname, gender, dob, jdate, ldate, address,email,tel FROM users, friendshippending WHERE inviteeid=? and inviterid = userid";
	...
	public int rejectFriend(int inviterID, int inviteeID) {
		...
		String query = "DELETE FROM friendshippending WHERE inviterid=? and inviteeid= ?";
		try {
			if((preparedStatement = newCachedStatements.get(REJREQ_STMT)) == null)
	...
	public int CreateFriendship(int memberA, int memberB) {
		...
		try {
			...
			String DML = "INSERT INTO friendshipconfirm values(?,?)";
	...
	public int inviteFriend(int inviterID, int inviteeID) {
		...
		if(inviterID < 0 || inviteeID < 0)
			return -1;
		String query = "INSERT INTO friendshippending values(?,?)";
		try {
	...
	public int thawFriendship(int friendid1, int friendid2){
		...
		String query = "DELETE FROM friendshipconfirm WHERE (inviterid=? and inviteeid= ?) OR (inviterid=? and inviteeid= ?)";
		try {
	...
	public HashMap<String, String> getInitialStats() {
		...
		try {
			...
			query = "select count(*) from friendshipconfirm where (inviterid="+Integer.parseInt(offset) +" OR inviteeid="+Integer.parseInt(offset) +")" ;
			rs = st.executeQuery(query);
			if(rs.next()){
			...
			if(rs != null) rs.close();
			query = "select count(*) from friendshippending where (inviteeid="+Integer.parseInt(offset) +")" ;
			rs = st.executeQuery(query);
			if(rs.next()){
	...
	public int queryPendingFriendshipIds(int inviteeid, Vector<Integer> pendingIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT inviterid from friendshippending where inviteeid='"+inviteeid+"'";
			rs = st.executeQuery(query);
			while(rs.next()){
	...
	public int queryConfirmedFriendshipIds(int profileId, Vector<Integer> confirmedIds){
		...
		try {
			st = conn.createStatement();
			query = "SELECT inviterid, inviteeid from friendshipconfirm where (inviteeid="+profileId+" OR inviterid="+profileId+")";
			rs = st.executeQuery(query);
			while(rs.next()){
	...
	public void createSchema(Properties props){
		...
		try {
			...
			dropTable(stmt, "friendshipconfirm");
			dropTable(stmt, "friendshippending");
		   ...
		   //stmt.executeUpdate("CREATE SEQUENCE  USERIDINC  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 21 CACHE 20 NOORDER  NOCYCLE ");
		    ...
		    /**************************Modified*******************************************/
			stmt.executeUpdate("CREATE TABLE FRIENDSHIPCONFIRM (INVITERID INTEGER, INVITEEID INTEGER)");
			stmt.executeUpdate("CREATE TABLE FRIENDSHIPPENDING (INVITERID INTEGER, INVITEEID INTEGER)");
			/**************************Modified*******************************************/
			...
			stmt.executeUpdate("ALTER TABLE USERS MODIFY USERID INTEGER NOT NULL");
			...
			stmt.executeUpdate("ALTER TABLE MANIPULATION MODIFY MID INTEGER NOT NULL ");
			stmt.executeUpdate("ALTER TABLE MANIPULATION MODIFY CREATORID INTEGER NOT NULL ");
			stmt.executeUpdate("ALTER TABLE MANIPULATION MODIFY RID INTEGER NOT NULL ");
			stmt.executeUpdate("ALTER TABLE MANIPULATION MODIFY MODIFIERID INTEGER NOT NULL ");
			
			 /**************************Modified*******************************************/
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPCONFIRM ADD CONSTRAINT FRIENDSHIPC_PK PRIMARY KEY (INVITERID, INVITEEID)");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPCONFIRM MODIFY INVITERID INTEGER NOT NULL");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPCONFIRM MODIFY INVITEEID INTEGER NOT NULL");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPPENDING ADD CONSTRAINT FRIENDSHIPP_PK PRIMARY KEY (INVITERID, INVITEEID)");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPPENDING MODIFY INVITERID INTEGER NOT NULL");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPPENDING MODIFY INVITEEID INTEGER NOT NULL");
			
			 /**************************Modified*******************************************/
			
			
			stmt.executeUpdate("ALTER TABLE RESOURCES ADD CONSTRAINT RESOURCES_PK PRIMARY KEY (RID) ");
			stmt.executeUpdate("ALTER TABLE RESOURCES MODIFY RID INTEGER NOT NULL ");
			stmt.executeUpdate("ALTER TABLE RESOURCES MODIFY CREATORID INTEGER NOT NULL ");
			stmt.executeUpdate("ALTER TABLE RESOURCES MODIFY WALLUSERID INTEGER NOT NULL ");
			
			
			/*******************************************************************************************/
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPCONFIRM ADD CONSTRAINT FRIENDSHIPC_USERS_FK1 FOREIGN KEY (INVITERID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPCONFIRM ADD CONSTRAINT FRIENDSHIPC_USERS_FK2 FOREIGN KEY (INVITEEID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPPENDING ADD CONSTRAINT FRIENDSHIPP_USERS_FK1 FOREIGN KEY (INVITERID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE");
			stmt.executeUpdate("ALTER TABLE FRIENDSHIPPENDING ADD CONSTRAINT FRIENDSHIPP_USERS_FK2 FOREIGN KEY (INVITEEID)"
					+ "REFERENCES USERS (USERID) ON DELETE CASCADE");
			/*****************************************************************************************/
		...
		/*	stmt.executeUpdate("CREATE OR REPLACE TRIGGER RINC before insert on resources "
					...
					+ "end;");
			stmt.executeUpdate("ALTER TRIGGER RINC ");
			...
			stmt.executeUpdate("CREATE OR REPLACE TRIGGER UINC before insert on users "
					...
					+ "end;");
			stmt.executeUpdate("ALTER TRIGGER UINC "); */
			
			dropIndex(stmt, "RESOURCE_CREATORID");
			dropIndex(stmt, "RESOURCES_WALLUSERID");
			dropIndex(stmt, "FRIENDSHIPCONFIRM_INVITEEID");
			dropIndex(stmt, "FRIENDSHIPCONFIRM_INVITERID");
			dropIndex(stmt, "FRIENDSHIPPENDING_INVITEEID");
			dropIndex(stmt, "FRIENDSHIPPENDING_INVITERID");
   ...
   @Override
	public void buildIndexes(Properties props){
		...
		try {
			...
			stmt.executeUpdate("CREATE INDEX FRIENDSHIPCONFIRM_INVITEEID ON FRIENDSHIPCONFIRM (INVITEEID)"
					);
			stmt.executeUpdate("CREATE INDEX FRIENDSHIPPENDING_INVITEEID ON FRIENDSHIPPENDING (INVITEEID)"
					);
			...
			stmt.executeUpdate("CREATE INDEX FRIENDSHIPCONFIRM_INVITERID ON FRIENDSHIPCONFIRM (INVITERID)"
					);
			stmt.executeUpdate("CREATE INDEX FRIENDSHIPPENDING_INVITERID ON FRIENDSHIPPENDING (INVITERID)"
					);
